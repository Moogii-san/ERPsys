					...
					if(!empty($_POST['txtInvoiceId'])) {
						...
						if($result == 1) {
							...
							if(empty($row_selectInvoice)){
							   $error = "* Энэ ID-аар нэхэмжлэл алга";
							}
							else {
						...
						$row_selectInvoice = mysqli_fetch_array($result_selectInvoice);
						if(empty($row_selectInvoice)){
						   $error = "* Энэ хэрэглэгч дээр нэхэмжлэл алга";
						}
						else {
							mysqli_data_seek($result_selectInvoice,0);
						}
						...
						$row_selectInvoice = mysqli_fetch_array($result_selectInvoice);
						if(empty($row_selectInvoice)){
						   $error = "* Энэ огноогоор нэхэмжлэл алга";
						}
						else {
							mysqli_data_seek($result_selectInvoice,0);
						}
						...
						$error = "* Огноо ашиглан хайлт хийнэ үү";
					}
				}
				else {
					$error = "Сонголтоос сонгон уу.";
				}
			}
			else {
...
<body>
	...
	<section>
		...
		<option value="invoiceId">  Нэхэмжлэл Id </option>
		<option value="orderId"> Захиалга ID </option>
		...
		<select name="cmbRetailer" id="cmbRetailer" style="display:none;">
			<option value="" disabled selected>-- Хэрэглэгч сонгох --</option>
		...
		<table class="table_displayData" style="margin-top:20px;">
			...
			<tr>
				...
				<td> <a href="view_invoice_items.php?id=<?php echo $row_selectInvoice['invoice_id']; ?>">Тайлбар</a> </td>
			</tr>
			<?php } ?>
		</table>
		</form>
