		...
		if($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			if($phone != null) {
					...
					if(mysqli_query($con,$query_updateRetailer)) {
						echo "<script> alert(\"Амжилттай шинэчлэгдлээ\"); </script>";
						header("Refresh:0");
					}
					else {
					...
					$requireErr = "* Утасны дугаар заавал байх ёстой.";
				}
		}
	}
	else {
...
<body>
	...
	<section>
		...
		<ul class="form-list">
		<li>
			<div class="label-block"> <label for="retailer:phone">Дугаар</label> </div>
		...
		<li>
			<div class="label-block"> <label for="retailer:email">Майл хаяг</label> </div>
		...
		<li>
			<div class="label-block"> <label for="retailer:address">Хаяг</label> </div>
		...
		<li>
			<a href="change_password.php"><input type="button" value="Нууц үг солих" class="submit_button" /></a>
			<input type="submit" value="Профайл шинэчлэх" class="submit_button" /> <span class="error_message"> <?php echo $requireErr; ?> </span><span class="confirm_message"> <?php echo $confirmMessage; ?> </span>
		</li>
		</ul>
		</form>
	</section>
