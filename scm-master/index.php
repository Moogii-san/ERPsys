		...
		if(!empty($_POST['txtUsername']) && !empty($_POST['txtPassword']) && isset($_POST['login_type'])){
			...
			if($_SESSION['sessLogin_type'] == "retailer") {
				...
				if($row) {
					...
					$loginErr = "* Нэвтрэх нэр эсвэл нууц үг буруу байна..";
				}
			}
...
<body class="login-box">
	...
	<ul class="form-list">
	<li>
		<div class="label-block"> <label for="login:username">Нэвтрэх нэр</label> </div>
		<div class="input-box"> <input type="text" id="login:username" name="txtUsername" placeholder="Нэвтрэх нэр" /> </div>
	</li>
	<li>
		<div class="label-block"> <label for="login:password">Нууц үг</label> </div>
		<div class="input-box"> <input type="password" id="login:password" name="txtPassword" placeholder="Нууц үг" /> </div>
	</li>
	<li>
		<div class="label-block"> <label for="login:type">Нэвтрэх төрөл</label> </div>
		<div class="input-box">
		<select name="login_type" id="login:type">
		<option value="" disabled selected>-- Төрөл --</option>
		<option value="retailer">Хэрэглэгч</option>
		<option value="manufacturer">Үйлдвэрлэгч</option>
		<option value="admin">Админ</option>
		</select>
		</div>
	</li>
	<li>
		<input type="submit" value="Нэвтрэх" class="submit_button" /> <span class="error_message"> <?php echo $loginErr; echo $reqErr; ?> </span>
	</li>
	</ul>
	</form>
</body>
